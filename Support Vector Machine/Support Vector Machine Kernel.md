...
# Define different kernel types to test
kernel_types = ['linear', 'rbf', 'sigmoid']

for kernel in kernel_types:
    # Create SVM model with the specified kernel type
    model_pu = SVC(random_state=42, kernel=kernel)
    model_do = SVC(random_state=42, kernel=kernel)
